...
#define kColorNAVI_BULE  [UIColor colorWithRed:39 / 255.f green:165/ 255.f blue:156 / 255.f alpha:1.f]
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view{
    [pickerView.subviews[1] setBackgroundColor:SWTRedColor];
    [pickerView.subviews[2] setBackgroundColor:SWTRedColor];
...
+ (void)showPickWithMakeSure:(void (^)(id, id, id))selectArea{
    ...
    if (!pick) {
        pick = [[YTDatePick alloc] initWithFrame:CGRectMake((SCREEN_WIDTH - Window_Root.width)/2+5, 0, Window_Root.width, Window_Root.height)];
        pick.tag = 999;
...
- (void)createButtonWithFrame:(CGRect )rect title:(NSString *)title action:(SEL)acrion{
    ...
    [cancleBtn setTitleColor:SWTRedColor forState:(UIControlStateNormal)];
...
- (void)makeSure{
    ...
    NSString* timeStr = [NSString stringWithFormat:@"%@-%@-%@",@(year.integerValue).string, @(month.integerValue).string, @(day.integerValue).string];
