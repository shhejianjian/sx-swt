...
- (CGRect)titleRectForContentRect:(CGRect)contentRect {
    ...
    return CGRectMake((CGRectGetWidth(contentRect)-CGRectGetWidth(rect))/2, CGRectGetHeight(contentRect)-CGRectGetHeight(rect)-17, CGRectGetWidth(rect), CGRectGetHeight(rect));
}
...
- (void)showPopoverWithItems:(NSArray<PopoverItem *> *)items forIndexPath:(NSIndexPath *)indexPath {
    if (items.count <= 0) {
        ...
        NSLog(@"Can not be more than %d items!!!",(NSInteger)CGRectGetWidth(self.frame)/80);
        return;
    }
